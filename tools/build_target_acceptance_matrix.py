...
def build_items() -> tuple[list[MatrixItem], dict[str, str]]:
    ...
    items = [
        ...
        MatrixItem("N02", "network", "DHCP is supported, with static IP fallback for deployment/debug.", "source static check; real DHCP not run", "PASS_SOURCE_NO_ETHERNET" if ps_lwip_static_ok else "MISSING", rel(ps_lwip_static), "Structured PS lwIP static report confirms DHCP start/wait/timeout, static fallback 192.168.1.10/24, TCP bridge readiness, and no-real-board boundary; real DHCP still requires Ethernet." if ps_lwip_static_ok else "Missing DHCP/static source evidence."),
